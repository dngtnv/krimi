<template>
  <v-row style="height:100%" align="center" justify="center">
    <v-col cols="12" lg="6" style="margin-top:auto;" :style="[!game.murdererChoice && 'margin-bottom:auto']">
      <h2 class="display-2 mb-4">{{ player.name }}</h2>
      <div class="display-1" v-if="game.murdererChoice">
        {{ murderer.name }} {{ t('used') }}
        <span class="blue--text">&nbsp;&nbsp;{{ game.murdererChoice.mean }}</span>
      ...
      <v-card v-else>
        <v-card-title style="word-break: inherit"
          >{{ murderer.name }} {{ t('is the murderer. Wait for means of murder and key evidence.') }}</v-card-title
        >
        <v-card-text>
          {{
            t(
          ...
          <v-btn
            ...
            :disabled="
              analysis.length < availableClues.length ||
                (game.forensicAnalysis && game.forensicAnalysis.length === availableClues.length)
            "
            >Send analysis</v-btn
          >
        </v-col>
...
export default {
  ...
  locales: {
    vn: {
      'is the murderer. Wait for means of murder and key evidence.':
        'là kẻ sát nhân. Chờ hung khí và bằng chứng quan trọng.',
      used: 'đã dùng',
      'to kill, and left behind': 'để phạm tội, và đã để lại',
      'as a key evidence.': 'như là bằng chứng quan trọng.',
      'Ask all the players, but the murderer to close their eyes. As soon as you receive the murderer clues, you can ask them to open their eyes again.':
        'Yêu cầu tất cả người chơi, chỉ trừ kẻ sát nhân hãy nhắm mắt. Cho đến khi bạn nhận được manh mối của kẻ giết người. Bạn có thể yêu cầu họ mở mắt.',
    },
  },
  props: {
    game: {
  ...
  computed: {
    availableClues() {
      return this.game.analysis.filter((item, index) => index < this.game.availableClues);
    },
    murderer() {
      const key = Object.keys(this.game.players).find((item) => this.game.players[item].index === this.game.murderer);
